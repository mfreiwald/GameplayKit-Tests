...
/// Relay control events though `ThumbStickControlDelegate`.
protocol ThumbStickControlDelegate: class {
...
class ThumbStickControl: SKSpriteNode {
    // MARK: Properties 
    
    /// The actual thumb pad that moves with touch.
    var touchPad: SKSpriteNode
    
    weak var delegate: ThumbStickControlDelegate?
    
    /// The center point of this `ThumbStickControl`.
    let center: CGPoint
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        delegate?.thumbStickControl(self, isPressed: true)
    }
    ...
    override func touchesMoved(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        for touch in touches {
            ...
            */
            ...
            delegate?.thumbStickControl(self, didUpdateXValue: normalizedDx, yValue: normalizedDy)
        }
    }
    ...
    func resetTouchPad() {
        ...
        let restoreToCenter = SKAction.moveTo(CGPoint.zeroPoint, duration: 0.2)
        touchPad.runAction(restoreToCenter)
        
        delegate?.thumbStickControl(self, isPressed: false)
        delegate?.thumbStickControl(self, didUpdateXValue: 0, yValue: 0)
    }
}
