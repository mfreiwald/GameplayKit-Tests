...
OS X 10.11 beta and iOS 9.0 beta have a known runtime issue when using GameplayKit behaviors on 32-bit iOS devices. To avoid this issue, run DemoBots on a 64-bit iOS device; in the iOS Simulator for a 64-bit device; or on OS X.
...
- "a" or "left arrow" rotates the PlayerBot anti-clockwise.
...
DemoBots uses the GameController framework to support third-party game controllers on both iOS and OS X.

When using a controller that matches the standard game controller profile:
...
When using a controller that matches the extended game controller profile:

- The d-pad and left thumbstick control the PlayerBot's movement.
- The right thumbstick controls the PlayerBot's orientation independently of its movement direction.
...
For more information about DemoBots, see WWDC 2015 session 609, "Deep Dive into DemoBots", available from https://developer.apple.com/wwdc/videos .
