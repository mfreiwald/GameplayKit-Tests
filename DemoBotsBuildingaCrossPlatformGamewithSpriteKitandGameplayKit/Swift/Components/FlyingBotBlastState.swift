...
class FlyingBotBlastState: GKState {
    ...
    func entitiesInRange() -> [GKEntity] {
        ...
        let entitiesInRange: [GKEntity] = entitySnapshot.entityDistances.filter {
            return $0.distance <= GameplayConfiguration.FlyingBot.blastRadius
        }.map {
            return $0.target
        }
    ...
    func performGoodBlast() {
        ...
        // Iterate throught the `TaskBot`s in range.
        for taskBot in taskBotsInRange {
            ...
            switch currentState {
                case is TaskBotAgentControlledState, is FlyingBotPreAttackState, is GroundBotRotateToAttackState, is GroundBotPreAttackState:
                    taskBot.isGood = true
