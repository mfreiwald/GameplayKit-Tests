...
class BeamComponent: GKComponent {
    ...
    */
    func findTargetInBeamArcWithCurrentTarget(currentTarget: TaskBot?) -> TaskBot? {
        ...
        let playerAntenna = AntennaInfo(entity: playerBot, antennaOffset: playerBot.antennaOffset)
        
        let botsInArc = snapshot.entityDistances.filter {
            // Filter out entities that aren't "bad" `TaskBot`s with a `RenderComponent`.
            guard let taskBot = $0.target as? TaskBot else { return false }
            guard let taskBotNode = taskBot.componentForClass(RenderComponent.self)?.node else { return false }
            if taskBot.isGood { return false }
            
            // Filter out `TaskBot`s that are too far away.
            if CGFloat($0.distance) > GameplayConfiguration.Beam.arcLength { return false }
            ...
            if playerAntenna.angleTo(taskBotAntenna) > Float(GameplayConfiguration.Beam.arcAngle / 2.0) {
                return false
            }

            // Filter out `TaskBot`s where there is scenery between their antenna and the `PlayerBot`'s antenna.
            var hasLineOfSite = true
            level.physicsWorld.enumerateBodiesAlongRayStart(playerAntenna.position, end: taskBotAntenna.position) { obstacleBody, _, _, stop in
                // Ignore this body if it belongs to the `TaskBot` or `PlayerBot`.
                if let entityNode = obstacleBody.node as? EntityNode {
                    if entityNode.entity == taskBot || entityNode.entity == playerBot {
                        return
                    }
                }
        ...
        if let currentTarget = currentTarget {
            if botsInArc.contains(currentTarget) { return currentTarget }
        }
        
        // Return the closest target in the beam's arc.
        return botsInArc.first
    }
}
